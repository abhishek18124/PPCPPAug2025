...
int upperBound(int arr[], int n, int t) {
	...
	while (s <= e) {
		int m = (s + e) / 2;
		if (arr[m] == t) {
			ans = m; // we've found the 't' at index 'm', save m and continue the search towards the right of 'm'
...
int main() {
	...
	int foIdx = lowerBound(arr, n, t);
	if (foIdx == -1) {
		// t is not present in the arr[]
		cout << 0 << endl;
	} else {
		// t is present in the arr[]
		int loIdx = upperBound(arr, n, t);
		cout << loIdx - foIdx + 1 << endl;
	}
