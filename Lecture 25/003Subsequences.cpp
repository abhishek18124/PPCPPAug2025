...
void f(char inp[], char out[], int i, int j) {
	...
	if (inp[i] == '\0') { // we've managed to build a subsequence
		out[j] = '\0';
		cout << out << endl;
		return;
	}
	...
	// f(i, j) = take decisions for remaining n-i characters
	// in the inp[] starting from index i s.t. the next free
	// slot in the out[] is at the jth index
