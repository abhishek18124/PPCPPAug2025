...
TreeNode* deleteFromBST(TreeNode* root, int key) {
	...
	if (key < root->val) {
		...
		// ask your friend to delete key from the leftSubtree

		root->left = deleteFromBST(root->left, key);

	} else if (key > root->val) {

		// ask your friend to delete key from the rightSubtree

		root->right = deleteFromBST(root->right, key);

	} else {

		// key is equal to root->val so delete the root node
		...
		if (root->left == NULL and root->right == NULL) {
			...
			// you want to delete a node with a single left child
			TreeNode* temp = root->left;
			delete root;
			root = temp;
		} else if (root->left == NULL and root->right != NULL) {
			// you want to delete a node with a single right child
