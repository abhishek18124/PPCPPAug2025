...
int computeDiameter(TreeNode* root) {
	...
	// 3. find the length of the longest path that goes through the root node

	int lenOfLongestPathThroughRoot = 2 + computeHeight(root->left) + computeHeight(root->right);

	return max(leftDia, max(rightDia, lenOfLongestPathThroughRoot));
...
Pair computeDiameterOptimised(TreeNode* root) {
	...
	// 1. ask your friend to find the diameter of the LST and simulataneously find the height of the LST
	Pair left = computeDiameterOptimised(root->left);

	// 2. ask your friend to find the diameter of the RST and simulataneously find the height of the RSt
	Pair right = computeDiameterOptimised(root->right);

	// 3. find the length of the longest path that goes through the root node
	int lenOfLongestPathThroughRoot = 2 + left.hgt + right.hgt;

	p.hgt = 1 + max(left.hgt, right.hgt);
	p.dia = max(left.dia, max(right.dia, lenOfLongestPathThroughRoot));
