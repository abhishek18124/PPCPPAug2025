...
void heapify(vector<int>& v, int n, int i) { // logn

	// fix the maxHeap prop. at the ith index

	int maxIdx = i; // assume value at index i is the largest
	...
	if (maxIdx != i) {
		...
		// maxHeap prop. was defintely not working at index i
...
int main() {
	...
	// 2. transform maxHeap into sorted array // time : O(nlogn)
	...
	// total space: O(logn) due to fn call stack of heapify
	// space can be optimised to O(1) if heapify is impl. iteratively
