...
int main() {
	...
	// time : O(n)
	// space: O(k) due to queue since queue maintains all negatives of a window
	...
	if (!q.empty()) {
		...
		// there is no negative in the 1st window
		ans.push_back(0);
	}
	...
	while (j < n) {
		...
		if (!q.empty()) {
			...
			// there is no negative in the current window
			ans.push_back(0);
		}
